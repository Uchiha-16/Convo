<?php
    class pagesM {
        ...
        public function getQuestions($tags) {
            ...
            FROM question JOIN user on question.userID = user.userID JOIN questiontag ON question.QID = questiontag.QID WHERE questiontag.tag IN (:tag)');
            $this->db->bind(':tag', $tags);
            $row = $this->db->resultSet();
            return $row;
        }
        ...
        public function getQuestionTags($QID) {
            $this->db->query('SELECT tag FROM questiontag WHERE QID = :QID');
